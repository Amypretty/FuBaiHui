...
//        header.put("sign", sign);
    ...
    public static String getUserAgent() {
        int versionCode = 1;
        try {
            versionCode = Application1901.getApplication().getPackageManager()
                    .getPackageInfo(Application1901.getApplication().getPackageName(), 0).versionCode;
        } catch (PackageManager.NameNotFoundException pE) {
            pE.printStackTrace();
        }
        String s = WebSettings.getDefaultUserAgent(Application1901.getAppContext())
                + " News/" + versionCode + " Android/"
